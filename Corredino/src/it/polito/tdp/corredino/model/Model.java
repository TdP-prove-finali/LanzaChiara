...
public class Model {
	...
	List<List<Product>> allRes = new LinkedList<>();
...
private void prova(List<Product> candidata, int L, float budget) {
	if(L==dao.totProdotti())
		return;

	for(List<Product> lp : categories.values()) {
		for(Product p: lp) {
			if(!candidata.contains(p))
			if(aggiuntaValida(p,candidata, budget) && esci==false) {
				candidata.add(p);
				prova(candidata, L+1, budget);
				candidata.remove(candidata.size()-1);
			} else if (tot>budget || esci==true)
				return;
		}
	}
...
private boolean aggiuntaValida(Product p, List<Product> candidata2, float budget) {
	...
	if(controllore.containsKey("Body"))
	if (controllore.get("Body")>5) {
		esci=true;
		return false;
	}
	if(controllore.containsKey("Ghettina"))
	if(controllore.get("Ghettina")>5){
		esci=true;
		return false;
	}
	if(controllore.containsKey("Calzini"))
	if(controllore.get("Calzini")>5) {
		esci=true;
		return false;
	}
	if(controllore.containsKey("Tutina"))
	...
	if(this.isCompleta(candidata2) && tot<budget) {
		allRes.add(candidata2);
		if(tot<totBest && budget-tot<50) {
			best= new ArrayList<>(candidata2);
			totBest=tot;
		}
...
private boolean isCompleta(List<Product> candidata2) {
	...
	if(controllore.get("Materasso culla")==1 )
		if(controllore.get("Cuscino culla")!=1 ||controllore.get("Copri materasso")<1)
			return false;
